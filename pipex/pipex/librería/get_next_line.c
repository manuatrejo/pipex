...
/*   Updated: 2024/08/31 21:04:10 by manguita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_joinfree(char *text, char *temp_line)
{
	...
	joined_str = ft_strjoin(text, temp_line);
	free (text);
	return (joined_str);
}
...
char	*read_line(int fd, char *text)
{
	...
	if (!text)
		text = ft_calloc(1, 1);
	temp_line = ft_calloc(sizeof(char), BUFFER_SIZE + 1);
	chars_read = 1;
	while (chars_read > 0)
	{
...
char	*rest(char *text)
{
	...
	while (text[i + j])
		j++;
	rest_of_line = ft_calloc(sizeof(char), j + 1);
	j = 0;
	while (text[i])
